          ...
          {agents.map((agent) => (
            <Card key={agent.id} className="p-6 border-border bg-white hover:shadow-xl transition-all relative group overflow-hidden">
              ...
              <div className="mt-6 pt-6 border-t border-border grid grid-cols-2 gap-3">
                <Button onClick={() => { setActiveTestAgent(agent); setIsTesting(true); }} className="bg-primary text-white font-bold text-xs gap-2 rounded-xl h-10 shadow-sm">
      ...
      <Dialog open={isTesting} onOpenChange={setIsTesting}>
        <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden rounded-3xl border-none shadow-2xl animate-in zoom-in-95 duration-200">
          ...
          <div className="p-4 bg-white border-t flex gap-2">
            <Input placeholder="Tanya sesuatu..." className="h-12 rounded-xl bg-gray-50 border-none focus-visible:ring-1 focus-visible:ring-primary" />
            <Button size="icon" className="h-12 w-12 bg-primary text-white rounded-xl shadow-lg shadow-primary/20"><Send className="w-5 h-5" /></Button>
          </div>
        </DialogContent>
      </Dialog>

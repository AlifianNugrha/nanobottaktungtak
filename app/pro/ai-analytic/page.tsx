      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-3">
            AI Analytics <div className="px-2 py-0.5 bg-orange-600/10 text-orange-600 text-[10px] rounded-md tracking-widest uppercase">Pro</div>
          </h1>
        ...
        <div className="flex gap-2">
          ...
          <Button className="bg-orange-600 text-white rounded-xl h-10 px-5 text-xs font-bold shadow-lg shadow-orange-600/20">
            Generate Report
          </Button>
        </div>
      </div>
        ...
        <Card className="lg:col-span-2 p-8 border-border shadow-sm space-y-8">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-lg flex items-center gap-2">
              <PieChart className="w-5 h-5 text-orange-600" /> Conversion Funnel
            </h3>
          ...
          <div className="space-y-4 py-4">
            {[
              { label: 'Total Conversations', value: '1,240', width: 'w-full', color: 'bg-orange-600' },
              { label: 'Leads Identified', value: '450', width: 'w-[65%]', color: 'bg-orange-600/70' },
              { label: 'Payment Link Generated', value: '180', width: 'w-[40%]', color: 'bg-orange-600/50' },
              { label: 'Closing / Paid', value: '82', width: 'w-[25%]', color: 'bg-orange-600/30' },
            ].map((step, i) => (
              <div key={i} className="space-y-2">
                <div className="flex justify-between text-xs font-bold">
                  <span>{step.label}</span>
                  <span className="text-orange-600">{step.value}</span>
                </div>
            ...
            <div className="relative z-10 space-y-4">
              ...
              <div className="pt-2">
                <p className="text-[11px] text-slate-400 leading-relaxed font-medium">
                  <span className="text-orange-600 font-bold">Rekomendasi AI:</span> Aktifkan notifikasi stok rendah untuk Produk X dan buat promo khusus jam malam.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between group">
                  ...
                  <span className="text-xs font-mono font-bold text-orange-600 bg-orange-600/5 px-2 py-1 rounded-lg">
                    {item.pct}
                  </span>
                </div>
              ))}
